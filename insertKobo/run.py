...
def get_diff_uuids(config):
    # Establecemos los parámetros y obtenemos los UUIDs
    ...
    # Filtrado directo usando my_uuids_to_transfer
    filtered_src_uuids = [uuid for uuid in src_uuids if uuid in my_uuids_to_transfer]
    print(f"UUIDs después del filtrado: {filtered_src_uuids}")  # Verificar los UUIDs filtrados
    return list(set(filtered_src_uuids).difference(set(dest_uuids)))
